...
export function createRoom(hostId: string, hostName: string): { roomCode: string; state: GameState } {
...
export function joinRoom(roomCode: string, playerId: string, playerName: string): { success: boolean; state?: GameState; error?: string } {
    ...
    if (room.state.players.find(p => p.id === playerId)) {
        return { success: true, state: getPlayerView(room.state, playerId) };
    }
...
export function startGame(roomCode: string, playerId: string): { success: boolean; error?: string } {
    ...
    if (room.state.players.length < 2) return { success: false, error: 'Need at least 2 players' };
...
function postBlind(room: Room, playerIndex: number, amount: number): void {
...
export function handleAction(roomCode: string, playerId: string, action: PlayerAction): { success: boolean; error?: string } {
    const room = rooms.get(roomCode);
    if (!room) return { success: false, error: 'Room not found' };
    ...
    if (playerIndex !== room.state.currentPlayerIndex) return { success: false, error: 'Not your turn' };
    ...
    switch (action.type) {
        ...
        case 'call':
            if (toCall === 0) return { success: false, error: 'Nothing to call, check instead' };
        ...
        case 'raise':
            ...
            if (toAdd > player.chips) return { success: false, error: 'Not enough chips' };
            if (raiseAmount < room.state.minRaise && toAdd < player.chips) return { success: false, error: 'Raise too small' };
            player.chips -= toAdd;
            player.bet = totalBet;
...
function checkRoundComplete(room: Room): void {
    ...
    do {
        ...
        (room.state.players[room.state.currentPlayerIndex].folded ||
...
function determineWinner(room: Room): void {
    room.state.phase = 'showdown';
    const activePlayers = room.state.players.filter(p => !p.folded);

    const handResults = activePlayers.map(p => ({
        player: p,
        result: evaluateHand(p.cards, room.state.communityCards)
    }));

    handResults.sort((a, b) => compareHands(b.result, a.result));

    // Find all winners (ties)
    const winners = handResults.filter(hr => compareHands(hr.result, handResults[0].result) === 0);
...
export function getRoomState(roomCode: string, playerId: string): GameState | null {
    const room = rooms.get(roomCode);
    if (!room) return null;
    return getPlayerView(room.state, playerId);
}

export function getFullRoomState(roomCode: string): GameState | null {
    const room = rooms.get(roomCode);
    return room?.state || null;
}

export function newHand(roomCode: string): boolean {
    const room = rooms.get(roomCode);
    if (!room) return false;
    if (room.state.phase !== 'showdown' && room.state.phase !== 'ended') return false;
    startNewHand(room);
    return true;
}

export function playerDisconnected(playerId: string): string | null {
...
export function playerReconnected(playerId: string, roomCode: string): boolean {
    const room = rooms.get(roomCode);
    if (!room) return false;
